    ...
    class Rsync < Ssh
      ...
      class Connection < Ssh::Connection
        def upload(locals, remote)
          ...
          logger.info(format("[rsync] Time taken to upload #{rsync_candidates.join(";")} to #{self}:#{remote}: %.2f sec", time))
          unless ret.first
            logger.warn("[rsync] rsync exited with status #{$CHILD_STATUS.exitstatus}, using SCP instead")
            @rsync_failed = true
          end
